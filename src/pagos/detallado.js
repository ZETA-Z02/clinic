...
export default class PresupuestoDetallado extends ApiService {
    constructor() {
        super();
        this.idcliente = $("#idcliente").val();
        this.controller = "pagos";
        this.$contenedor = $("#tabla-detallado");
    }
    async init() {
        await this.getPresupuestoDetallado();
        ...
        this.calcularDeuda("#monto-pagar-detallado", "#importe-detallado", "#mostrar-deuda-detallado");
        this.calcularDeudaTabla();
        // Habilitar modificaciones
        this.habilitarModificacion();
        await this.generarDoctores();
    }
    async getPresupuestoDetallado() {
        try {
            let total_mostrar = 0,
                pagado_mostrar = 0,
                deuda_mostrar = 0;
            // Obteniendo el id del cliente
            const data = await this.readOne({ id: this.idcliente, tipo: "detallado" },this.controller,"presupuestos");
            const tbody = this.$contenedor.find("#tbody-detallado");
            ...
            $(`#detallado-total`).html(`s/. ${total_mostrar}`);
            $(`#detallado-deuda`).html(`s/. ${deuda_mostrar}`);
            $(`#detallado-pagado`).html(`s/. ${pagado_mostrar}`);
            tbody.append(html);
        } catch (error) {
            ...
            try{
              ...
              timeout = setTimeout(async () => {
                  ...
                  if (Object.values(data).every((value) => value.trim() !== "")) {
                      ...
                      pieza.val("");
                      doctor.val("");
                      await that.getPresupuestoDetallado();
    ...
    habilitarModificacion(){
      this.$contenedor.on("click","#btn-modificar-detallado", async () => {
        // HABILITAR DELETE
        let fila = $(".row-trash");
